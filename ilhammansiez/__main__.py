...
from ilhammansiez import dispatcher, updater, TOKEN, WEBHOOK, OWNER_ID, CERT_PATH, PORT, URL, LOGGER, \
    ALLOW_EXCL
...
TECHNO_IMG = "https://telegra.ph/file/924bd9752cd31dd8394fb.jpg"
IMPORTED = {}
MIGRATEABLE = []
HELPABLE = {}
STATS = []
...
def test(bot: Bot, update: Update):
    ...
    # update.effective_message.reply_text("Halo tester! _I_ *have* `markdown`", parse_mode=ParseMode.MARKDOWN)
...
def start(bot: Bot, update: Update, args: List[str]):
    if update.effective_chat.type == "private":
        ...
        update.effective_message.reply_text("Hi saya kembali  ONline")
...
def help_button(bot: Bot, update: Update):
    ...
    try:
        if mod_match:
            ...
            text = "Disini untuk petertolongan module *{}* module:\n".format(HELPABLE[module].__mod_name__) \
...
def settings_button(bot: Bot, update: Update):
    ...
    try:
        if mod_match:
            ...
            chat = bot.get_chat(chat_id)
            query.message.reply_text(text="Halo yang disana! Ada beberapa pengaturan untuk {} - lanjutkan dan pilih apa "
...
def get_settings(bot: Bot, update: Update):
    ...
    if chat.type != chat.PRIVATE:
        if is_user_admin(chat, user.id):
            ...
            text = "Klik disini untuk setting."
